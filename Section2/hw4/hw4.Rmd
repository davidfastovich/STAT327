...
```{r}
# identify(x = area$land, y = area$farm, n = 1)
# identify(x = area$land, y = area$farm, n = 1)
outlier.1 <- 43
outlier.2 <- 2
```
...
for (i in 1:dim(area)[1]) {
  fit.area.jack <- lm(farm ~ land, data = area[-i, ])
  pred <- predict(fit.area.jack, newdata = data.frame(land = area$land[i]))
  resid.jack <- area$farm[i] - pred
  r.jack <- rbind(r.jack, resid.jack)
}
```

Plot these "jackknife" residuals.
```{r}
plot(x = predict(fit.area), y = r.jack$X.12031.1567752233, xlab = "Fitted Values for Original Linear Model", ylab = "Jackknife Residuals", ylim = c(-400000, 400000))
abline(h = 0, lty = 2)
```
...
```{r}
ggplot(warpbreaks, aes(breaks)) +
  geom_density(aes(color = wool))
```

**Wool type does have a strong effect on the number of breaks. Wool B has much less breaks when compared to Wool B.**
...
```{r}
ggplot(warpbreaks, aes(breaks)) +
  geom_density(aes(color = tension))
```

**H tension tended to produced the least amount of breaks, followed by M tension, and L tension.**
...
**The most striking feature of the the plot is the grouping of the data. When the eruption time is greater than about 4 minutes the waiting time tends to be above 70 minutes. However, when the eruption time is less than 3 minutes the waiting time tends to be below 70 minutes.**
...
movies.lines <- grep(pattern = "\\s<a href=\"/title/(.*)/.*", x = movies, value = TRUE)
movie.link <- sub(pattern = ".*<a href=\"(.*)/.*", replacement = "\\1", x = movies.lines)

producers <- list()

for (i in 1:length(movie.link)) {
  link <- paste0("http://www.imdb.com/", movie.link[i], "/fullcredits")
  print(link)
  link.lines <- readLines(link)
  title.line <- grep(pattern = ".*<title>(.*)</title>.*", x = link.lines, value = TRUE)
  title <- sub(pattern = ".*<title>(.*)\\s[(].*", replacement = "\\1", x = title.line)
  tables <- readHTMLTable(link, stringsAsFactors = FALSE)[[4]]
