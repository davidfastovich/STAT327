...
```{r}
tables = readHTMLTable(getURL("https://simple.wikipedia.org/wiki/List_of_U.S._states_by_area"), stringsAsFactors = FALSE)
tables <- tables[[2]]
colnames(tables) <- c("rank", "state", "sq.km", "sq.miles")
tables$sq.km <- gsub(pattern = ",", replacement = "", x = tables$sq.km)
tables$sq.miles <- gsub(pattern = ",", replacement = "", x = tables$sq.miles)
```
