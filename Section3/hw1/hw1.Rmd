...
d.robust.regr <- function(x) {
  ...
  k <- 19000
  t <- (y - b0 - (b1*x))
  d.db <- ifelse((abs(t) <= k), (t^2), (2*k*sign(t)))
...
fore <- optimize(forecast.error, interval = c(0,1))
fore$minimum     
```
...
plot(nhtemp)
lines((fore$minimum * nhtemp)+((1 - fore$minimum)*y.hat.val), col = "blue") 
```
...
plot(nhtemp)
lines((fore$minimum * nhtemp)+((1 - fore$minimum)*y.hat.val), col = "blue")
lines((.1 * nhtemp)+((1 - .1)*y.hat.1), col = "red")
lines((.9 * nhtemp)+((1 - .9)*y.hat.9), col = "green")
...
optim(c(1, 1), mle, method = "Nelder-Mead")
...
curve(p.b0.b1, from = min(menarche.cases$age), to = max(menarche.cases$age))
points(jitter(x = menarche.cases$age), y = menarche.cases$reached.menarche)
