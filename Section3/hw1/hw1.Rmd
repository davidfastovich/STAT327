...
robust.regr <- function(beta, x, y, k) {
  return((1/50) * sum(rho(t = (y - beta[1] - (beta[2]*x)), k =19000)*(y - beta[1] - )
}
...
optim(par = c(0,0), fn = robust.regr, method = "Nelder-Mead")

rlm(farm ~ land, data = area)

# robust.regr.b0 <- function(x) {
#   b0 <- x[1]
#   b1 <- x[1]
#   return((-1/n)*sum(r.prime*(y-bo-b1*x)))
# }
# 
# robust.regr.b1 <- function(x) {
#   y = area$farm
#   x = area$land
#   n = 50
#   b0 <- x[1]
#   b1 <- x[1]
#   return((-1/n)*sum(x*r.prime*(y-bo-b1*x)))
# }
# 
# robust.regr.gr <- function(x) {
#   k = 19000
#   return(c(2*t, 2*k*sign(t)))
# }
