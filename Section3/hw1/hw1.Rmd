...
forecast.error <- function(b) {
  ...
  y.hat = y[1]
  for (i in 2:length(y)) {
    y.hat[i] = (b*y[i-1]) + ((1-b)*y.hat[i-1])
  }
  return(sum((y - y.hat)^2)/60)
}
...
y.hat <- function(y){
  y.hat = y[1]
  for (i in 2:length(y)) {
    y.hat[i] = (fore$minimum*y[i-1]) + ((1-fore$minimum)*y.hat[i-1])
  }
}


plot(nhtemp)
lines((fore$minimum * nhtemp)+((1 - fore$minimum)*y.hat), col = "blue") 
```
...
plot(nhtemp)
lines((fore$minimum * nhtemp)+((1 - fore$minimum)*y.hat), col = "blue") 
lines((.1 * nhtemp)+((1 - .1)*y.hat), col = "red") 
lines((.9 * nhtemp)+((1 - .9)*y.hat), col = "green") 
```
